...
`gh auth status` を読み取り確認として実行する。通常のローカル実行経路とは、
その後の GitHub 操作にも使え、OS の資格情報ストア（例: `Keychain/keyring`）へ
到達できる経路を指す。トークン値は表示、転記、保存しない。`gh auth token` の
実行や、`gh auth status` の出力に含まれる Token 行の共有・記録もしない。

Codex などで実行経路を指定できる場合は、最初の `gh auth status` から通常の
ローカル実行経路を明示して要求する。既定の sandbox で試してから通常経路へ
切り替える手順にはしない。通常経路の実行が許可されない、または利用できない
場合、認証状態は未確定として止める。隔離経路のエラー文を、利用者へ
「トークンが無効」「認証が切れた」と案内せず、`gh auth login` も案内しない。
利用者向けの認証診断は、必ずこの節の判定に従う。
